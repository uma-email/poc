...
const useGmailAPI = () => {
  ...
  const getLabels = useCallback(() => {
    ...
    gmailApi.users.labels.list({ userId }).then(({ result }) => {
      const systemAll = result.labels.filter((label) => label.type === 'system')
      const category = systemAll.filter((label) => label.id.startsWith('CATEGORY')).filter((label) => !label.id.endsWith('PERSONAL'))
  ...
  const loadMails = useCallback(() => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.threads
      .list({ userId, maxResults: 10000, includeSpamTrash: true })
      .then(({ result }) => Promise.all(result.threads.map(({ id }) => gmailApi.users.threads.get({ userId, id }))))
      .then((responses) => {
  ...
  const batchDeleteMessages = useCallback((ids) => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.messages.batchDelete({ ids, userId }).execute()
  }, [])

  const loadDrafts = useCallback(() => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.drafts
      .list({ userId })
      .then(({ result }) => Promise.all((result.drafts || []).map(({ id }) => gmailApi.users.drafts.get({ userId, id }))))
      .then((responses) => {
        const drafts = responses.map(({ result }) => result).reduce((accum, current) => ({ ...accum, [current.id]: current }), {})
        updateDrafts(drafts)
      })
  }, [])
  ...
  const createDraft = useCallback((draft) => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.drafts.create({ userId, message: { raw: draft.content } }).then(({ result }) => newDraftEdit({
      id: result.id,
      sender: userId,
      ...draft,
    }))
  ...
  const sendDraft = useCallback((id) => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.drafts.send({ id, userId }).then(() => closeDraftEdit(id))
  })
  const deleteDraft = useCallback((id) => {
    const userId = user.emailAddresses[0].value
    gmailApi.users.drafts.delete({ id, userId }).then(() => closeDraftEdit(id))
  })
  return {
    loadMails,
    getLabels,
