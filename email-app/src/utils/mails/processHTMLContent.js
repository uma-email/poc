...
const processHTMLContent = (scope, raw) => {
  ...
  const stripAndAddScope = (selectors) => {
    selectors.filter((selector) => selector.toLowerCase !== 'body').map((selector) => `#${scope} ${selector}`)
  }
  stylesheets.forEach((stylesheet) => {
    try {
      const css = cssParser.parse(stylesheet.innerText)
      Object.values(css.stylesheet.rules).forEach((rule) => {
        if (rule.type === 'rule') {
