...
export const DraftsWrapper = (WrappedComponent) => (props) => {
...
/* export const DraftsWrapper = WrappedComponent => {
  return props => {
    const [drafts, dispatch] = useReducer(reducer, {
      drafts: [],
      editing: {},
    ...
    const updateDraftEdit = useActionCreator(actions.updateDraftEdit, dispatch)
    return (
        <WrappedComponent drafts newDraftEdit closeDraftEdit updateDraftEdit updateDrafts {...props} />
    );
  };
}; */
