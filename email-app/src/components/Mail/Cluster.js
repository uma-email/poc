...
import {
  withStyles, Accordion, AccordionSummary, AccordionDetails, Typography, Avatar, colors,
...
const Cluster = ({
  ...
  const flattenThreads = Object.values(threads).flatMap((thread) => thread.threads)
  ...
  return (
    <Accordion expanded={expanded} onChange={() => setExpanded((exp) => !exp)} className={classNames(expanded && classes.expanded)}>
          ...
          <>
            <div className={classes.sender}>
              ...
              <Typography className={classNames(classes.name, hasUnread && classes.unread)}>
                ...
                {threadCount > 1 && <span className={classes.threadCount}>{`(${threadCount})`}</span>}
              </Typography>
            </div>
            ...
            <div className={classes.actions}>
              {actions.backToInbox && <InboxIcon className={classNames(classes.actionIcon, classes.iconInbox)} onClick={backToInbox} />}
              {actions.markAsDone && <CheckIcon className={classNames(classes.actionIcon, classes.iconDone)} onClick={markAsDone} />}
              {actions.trash && <DeleteIcon className={classes.actionIcon} onClick={trash} />}
              {actions.permanentDelete && <DeleteIcon className={classes.actionIcon} onClick={permanentDelete} />}
            </div>
          </>
        )}
      </AccordionSummary>
      <AccordionDetails className={classes.content}>
        {Object.values(threads).map((nested) => (
          <div key={nested.label} className={classes.nested}>
            <Typography variant='subtitle1' className={classes.nestedTitle}>
              {t(`date:${nested.label}`, { date: nested.date })}
            </Typography>
            {nested.threads.map((thread) => (
