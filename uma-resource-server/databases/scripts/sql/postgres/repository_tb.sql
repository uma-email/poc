...
ALTER TABLE ONLY repository.acl
    ADD CONSTRAINT acl_id_email_unique UNIQUE (id, email);
...
CREATE TRIGGER repository_acl_updated
    BEFORE UPDATE ON repository.acl
    FOR EACH ROW
    EXECUTE PROCEDURE repository.acl_updated ();
...
CREATE FUNCTION repository.file_content_table_inserted ()
    RETURNS TRIGGER
    LANGUAGE plpgsql
    SECURITY DEFINER
    AS $$
...
CREATE OR REPLACE FUNCTION repository.prevent_update ()
    RETURNS TRIGGER
    AS $BODY$
BEGIN
...
CREATE TRIGGER email_file_inserted
    BEFORE INSERT ON repository.file
    FOR EACH ROW
    EXECUTE PROCEDURE repository.file_table_inserted ();

CREATE TRIGGER email_file_content_inserted
    BEFORE INSERT ON repository.file_content
    FOR EACH ROW
    EXECUTE PROCEDURE repository.file_content_table_inserted ();

CREATE TRIGGER email_prevent_update
    BEFORE UPDATE ON repository.file
    FOR EACH ROW
    EXECUTE PROCEDURE repository.prevent_update ();

CREATE TRIGGER email_prevent_update
    BEFORE UPDATE ON repository.file_content
    FOR EACH ROW
    EXECUTE PROCEDURE repository.prevent_update ();
