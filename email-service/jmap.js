go.Jmap = {
  ...
  */
  upload: function (file, cfg) {
    ...
    var notifyEl = uploadNotification.addUpload({
      items: [
    ...
    queueItem.transactionId = Ext.Ajax.request({
      ...
      progress: function (e) {
        if (e.lengthComputable) {
          ...
          if (notifyEl) {
            ...
            if (box) { box.child('em', true).innerText = Math.round(queueItem.remainingSeconds) + t('s') }
      ...
      failure: function (response, options) {
        var data = response
        var title = response.isAbort ? t('Upload aborted') : t('Upload failed')
  ...
  */
  sse: function () {
    ...
    try {
      ...
      var url = go.User.eventSourceUrl + '?types=' + entities.column('name').join(',')

      var source = new EventSource(url); var me = this
  ...
  */
  ...
  scheduleRequest: function (options) {
    var clientCallId = 'clientCallId-' + this.nextCallId(); var promise = new Promise(function (resolve, reject) {
    ...
    Ext.Ajax.request({
      ...
      success: function (response, opts) {
        ...
        responses.forEach(function (response) {
          ...
          var o = this.requestOptions[clientCallId]; var me = this
          if (!o) {
            // aborted
